...
class UserMetaSyncTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->tag_manager_mock->shouldReceive('normalize_tag_input')->andReturnUsing(
            function ( $tags ) {
                return [ 'ids' => $tags, 'names' => $tags, 'pairs' => [] ];
            }
        )->byDefault();
        $this->tag_manager_mock->shouldReceive('prepare_tags_for_payload')->andReturnUsing(
            function ( $ids ) {
                return $ids;
            }
        )->byDefault();
    ...
    public function test_stores_contact_id_and_last_sync_for_user(): void
    {
        ...
        Functions\when('update_user_meta')->alias(
            function ( $uid, $key, $val ) use ( &$last_sync_stored ) {
    ...
    public function test_flushes_pending_tags_for_user_item(): void
    {
        ...
        Functions\when('delete_user_meta')->alias(
            function ( $uid, $key ) use ( &$deleted_keys ) {
                $deleted_keys[] = $key;
                return true;
            }
        );
    ...
    public function test_does_not_flush_pending_for_wc_items(): void
    {
        ...
        Functions\when('get_user_meta')->alias(
            function ( $uid, $key, $single ) use ( &$pending_checked ) {
                if ($key === '_ghl_pending_tags' ) {

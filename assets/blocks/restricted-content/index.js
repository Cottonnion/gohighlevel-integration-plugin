...
( function () {
	...
	function getRuleLabel( rule ) {
		switch ( rule ) {
			case 'any':  return __( 'Has ANY of these tags', 'ghl-crm-integration' );
			case 'all':  return __( 'Has ALL of these tags', 'ghl-crm-integration' );
			case 'none': return __( 'Does NOT have these tags', 'ghl-crm-integration' );
			default:     return __( 'Restricted', 'ghl-crm-integration' );
		}
	}
	...
	blocks.registerBlockType( 'ghl-crm/restricted-content', {
		...
		description : __( 'Control content visibility based on GoHighLevel contact tags', 'ghl-crm-integration' ),
		icon        : 'lock',
		category    : 'ghl-crm',
		example     : {
			...
			innerBlocks: [ {
				name      : 'core/paragraph',
				attributes: { content: __( 'This content is restricted to VIP members only', 'ghl-crm-integration' ) },
			} ],
		},
		attributes: {
		...
		edit: function ( props ) {
			...
			var isBlockSelected = useSelect ? useSelect( function ( select ) {
			...
			}, [ isBlockSelected, selectId ] ); // tags intentionally excluded — handled by Select2 change event

			// ─── Not Connected ───────────────────────────────────────────────────
			if ( ! blockData.connected ) {
				return el( 'div', { className: 'ghl-restricted-content-block' },
					el( Placeholder, {
						icon        : 'lock',
						label       : __( 'Restricted Content', 'ghl-crm-integration' ),
						instructions: __( 'Please connect to GoHighLevel in plugin settings to use content restrictions.', 'ghl-crm-integration' ),
					},
						el( 'a', {
			...
			// ─── Tag Selector ────────────────────────────────────────────────────
			var tagSelector = el( 'div', { className: 'ghl-tag-selector-wrapper' },
				...
				}, __( 'Search for tags or type to create new ones', 'ghl-crm-integration' ) )
			);

			// ─── Inspector ───────────────────────────────────────────────────────
			var inspector = el( InspectorControls, null,
				el( PanelBody, { title: __( 'Access Rules', 'ghl-crm-integration' ), initialOpen: true },
					el( SelectControl, {
						...
						options : [
							...
							{ label: __( 'User does NOT have these tags', 'ghl-crm-integration' ), value: 'none' },
						],
						onChange: function ( v ) { setAttrs( { rule: v } ); },
						help    : __( 'Choose how tags should be checked for access', 'ghl-crm-integration' ),
					} ),
					tagSelector
				),
				el( PanelBody, { title: __( 'Fallback Settings', 'ghl-crm-integration' ), initialOpen: false },
					el( ToggleControl, {
						label   : __( 'Show Fallback Message', 'ghl-crm-integration' ),
						checked : attrs.showMessage,
						onChange: function ( v ) { setAttrs( { showMessage: v } ); },
						help    : __( 'Display a message when user does not have access', 'ghl-crm-integration' ),
					} ),
					attrs.showMessage && el( Fragment, null,
						el( TextareaControl, {
							label   : __( 'Fallback Content', 'ghl-crm-integration' ),
							value   : attrs.fallbackContent,
							onChange: function ( v ) { setAttrs( { fallbackContent: v } ); },
							help    : __( 'Message shown to users without access (supports HTML)', 'ghl-crm-integration' ),
							rows    : 4,
						} ),
						el( 'div', { className: 'ghl-fallback-style-controls' },
							el( 'div', { style: { marginBottom: '12px' } },
								el( 'label', { style: { display: 'block', fontWeight: '600', marginBottom: '6px' } }, __( 'Background Color', 'ghl-crm-integration' ) ),
								el( ColorPalette, {
									value   : attrs.fallbackBgColor,
									onChange: function ( color ) { setAttrs( { fallbackBgColor: color || '#fff3cd' } ); },
								} )
							),
							el( 'div', { style: { marginBottom: '12px' } },
								el( 'label', { style: { display: 'block', fontWeight: '600', marginBottom: '6px' } }, __( 'Text Color', 'ghl-crm-integration' ) ),
								el( ColorPalette, {
									value   : attrs.fallbackTextColor,
									onChange: function ( color ) { setAttrs( { fallbackTextColor: color || '#856404' } ); },
								} )
							),
							el( 'div', { style: { marginBottom: '12px' } },
								el( 'label', { style: { display: 'block', fontWeight: '600', marginBottom: '6px' } }, __( 'Border Color', 'ghl-crm-integration' ) ),
								el( ColorPalette, {
									value   : attrs.fallbackBorderColor,
									onChange: function ( color ) { setAttrs( { fallbackBorderColor: color || '#ffc107' } ); },
								} )
							),
							el( RangeControl, {
								label   : __( 'Padding (px)', 'ghl-crm-integration' ),
								value   : attrs.fallbackPadding,
								onChange: function ( value ) { setAttrs( { fallbackPadding: value } ); },
								min     : 0,
								max     : 64,
								step    : 1,
							} )
			...
			var tagLabel = tagCount === 1
				? __( 'tag', 'ghl-crm-integration' )
				: __( 'tags', 'ghl-crm-integration' );

			var restrictionIndicator = el( 'div', {
				...
				el( 'div', { style: { flex: 1 } },
					...
					el( 'span', { style: { fontSize: '13px', color: '#646970' } },
			...
			// ─── Fallback Preview ────────────────────────────────────────────────
			var fallbackPreview = attrs.showMessage && attrs.fallbackContent
				? el( 'div', {
					style: {
						marginTop   : '12px',
						padding     : ( attrs.fallbackPadding || 0 ) + 'px',
						background  : attrs.fallbackBgColor || '#fff3cd',
						border      : '1px solid ' + ( attrs.fallbackBorderColor || '#ffc107' ),
						borderRadius: '4px',
					},
				},
					el( 'strong', { style: { display: 'block', marginBottom: '4px' } },
					...
					el( 'div', { style: { fontSize: '13px', color: attrs.fallbackTextColor || '#856404' } },
			...
			return el( Fragment, null,
				inspector,
				el( 'div', { className: 'ghl-restricted-content-block' },
					restrictionIndicator,
					el( 'div', {
						style: { border: '1px dashed #ccc', padding: '16px', borderRadius: '4px' },
					},

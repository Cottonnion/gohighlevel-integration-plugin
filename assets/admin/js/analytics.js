...
(function ($) {
  ...
  function initDailyActivityChart(dailyActivity) {
    ...
    const successData = dailyActivityLabels.map(
      (date) => dailyActivity[date].success,
    );
    const failedData = dailyActivityLabels.map(
      (date) => dailyActivity[date].failed,
    );
  ...
  function initSuccessRateTrendChart(successFailureRates) {
    ...
    const successRateData = trendLabels.map(
      (date) => successFailureRates[date].success_rate,
    );
  ...
  function exportAnalyticsCSV(analyticsData) {
    ...
    Object.keys(daily_activity).forEach((date) => {
      csv.push([
        date,
        daily_activity[date].success,
        daily_activity[date].failed,
      ]);
    });
    csv.push([]);
    ...
    a.href = url;
    a.download =
      "ghl-analytics-" + new Date().toISOString().split("T")[0] + ".csv";

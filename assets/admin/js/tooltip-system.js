...
(function () {
  ...
  function handleDocumentInteraction(event) {
    ...
    if (
      activeElement &&
      target &&
      target.closest(`[${TOOLTIP_CONFIG.attribute}]`) === activeElement
    ) {
      return;
    }
  ...
  function initializeTooltips() {
    ...
    elements.forEach((element) => {
      ...
      element.addEventListener("touchstart", handleTooltipTouch, {
        passive: false,
      });
      ...
      if (
        ...
        !element.classList.contains("ghl-tooltip-icon")
      ) {
        ...
        if (mutation.type === "childList") {
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              if (
                node.hasAttribute &&
                node.hasAttribute(TOOLTIP_CONFIG.attribute)
              ) {
                shouldReinit = true;
              }
              // Check children

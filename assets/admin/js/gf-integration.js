/**
 * Gravity Forms GHL CRM Integration - Admin JavaScript
 *
 * Handles checkbox toggles, field mapping (loaded via AJAX), and Select2 tag input
 * on the Gravity Forms form-settings "GHL CRM" tab.
 */

(function ($) {
  "use strict";

  // Localized data (handle: syncly-gf-js → var: syncly_gf_js_data)
  const localized =
    typeof syncly_gf_js_data !== "undefined" ? syncly_gf_js_data : {};
  ...
  const GHL_GF_Admin = {
    ...
    bindEvents: function () {
      ...
      $("#syncly_gf_enabled").on("change", this.toggleSettings);
    ...
    toggleSettings: function () {
      ...
      $("#syncly_gf_settings_container").toggle(isEnabled);
    },
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success && response.data.fields) {
            GHL_GF_Admin.populateFieldDropdowns(response.data.fields);
          } else {
            GHL_GF_Admin.showFieldsError();
          }
        },
        error: function () {
          GHL_GF_Admin.showFieldsError();
    ...
    populateFieldDropdowns: function (fields) {
      $(".ghl-field-select").each(function () {
        ...
        // Add empty "do not sync" option first
        $select.append(
          $("<option></option>").attr("value", "").text("— Do not sync —"),
        );

        // Add each field as an option
        $.each(fields, function (key, label) {
          const $option = $("<option></option>").attr("value", key).text(label);
    ...
    checkEmailMapping: function () {
      ...
      $("#syncly_gf_email_notice").toggle(!hasEmail);
    },
    ...
    initTagsSelect2: function () {
      ...
      if (savedTags.length) {
        savedTags.forEach(function (tagName) {
          if (
            !$select[0].querySelector(
              'option[value="' + CSS.escape(tagName) + '"]',
            )
          ) {
            const option = new Option(tagName, tagName, false, false);
            $select.append(option);

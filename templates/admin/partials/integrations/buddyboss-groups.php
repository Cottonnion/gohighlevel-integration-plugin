	...
	<div style="background: #fff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 48px 32px; margin: 24px 0;">
		<div style="text-align: center; max-width: 600px; margin: 0 auto;">
			...
			<h3 style="margin: 0 0 12px; font-size: 24px; font-weight: 600; color: #1e293b;">
				<?php esc_html_e( 'BuddyBoss Platform Not Detected', 'syncly' ); ?>
			</h3>
			<p style="margin: 0 0 32px; font-size: 16px; color: #64748b; line-height: 1.6;">
				<?php esc_html_e( 'BuddyBoss Platform must be installed and activated with Groups component enabled to use this integration.', 'syncly' ); ?>
			</p>
			...
			<div style="display: flex; align-items: center; justify-content: center; gap: 12px; flex-wrap: wrap;">
				<?php if ( current_user_can( 'install_plugins' ) ) : ?>
					<button type="button" onclick="window.open('https://www.buddyboss.com/pricing', '_blank')" class="ghl-button ghl-button-primary" style="padding: 12px 24px; font-size: 15px;">
						<?php esc_html_e( 'Install BuddyBoss', 'syncly' ); ?>
					</button>
				<?php endif; ?>
...
<?php else : ?>
	...
	<div class="ghl-settings-section ghl-settings-card">
			...
			<div style="display: flex; align-items: center; gap: 16px; flex: 1;">
				...
				<div style="flex: 1;">
					<h2 style="margin: 0 0 6px 0; font-size: 20px; font-weight: 600; color: #1e293b; line-height: 1.2;">
						<?php esc_html_e( 'BuddyBoss Groups Integration', 'syncly' ); ?>
					</h2>
					<p class="description" style="margin: 0; font-size: 14px; color: #64748b; line-height: 1.5;">
						<?php esc_html_e( 'Sync BuddyBoss Groups to GoHighLevel Custom Objects and link members automatically', 'syncly' ); ?>
					</p>
				</div>
			</div>
			<div style="display: flex; align-items: center; gap: 12px; flex-shrink: 0;">
				...
				<label class="ghl-checkbox <?php echo $buddyboss_enabled ? 'is-checked' : ''; ?>" style="margin: 0; display: flex; align-items: center; gap: 8px;">
					...
					<span class="ghl-checkbox-label" style="font-weight: 500; color: #475569;">
						<?php echo $buddyboss_enabled ? esc_html__( 'Enabled', 'syncly' ) : esc_html__( 'Disabled', 'syncly' ); ?>
					</span>
				</label>
			</div>
		</div>
			...
			<p>
				...
				if ( defined( 'BP_PLATFORM_VERSION' ) ) {
					printf(
						...
						esc_html__( 'BuddyBoss Platform %s detected. Configure your integration settings below.', 'syncly' ),
						absint( BP_PLATFORM_VERSION )
					);
				} else {
					esc_html_e( 'BuddyBoss Groups detected. Configure your integration settings below.', 'syncly' );
				}
				?>
			</p>
			<hr>
			...
			<div class="ghl-checkbox-group" style="display: flex; flex-direction: column; gap: 12px;">
				<label class="ghl-checkbox <?php echo ! empty( $settings['buddyboss_auto_delete_custom_objects'] ) ? 'is-checked' : ''; ?>" style="display: flex; align-items: center; gap: 8px; margin: 0;">
					...
					<span class="ghl-checkbox-label">
						<?php esc_html_e( 'Auto-delete Custom Objects when group types are removed', 'syncly' ); ?>
						<span class="ghl-tooltip-icon" data-ghl-tooltip="<?php esc_attr_e( 'When enabled, deleting a BuddyBoss group type automatically removes its corresponding Custom Object schema from GoHighLevel. Disable to keep historical data even after group types are deleted.', 'syncly' ); ?>">?</span>
					</span>
				</label>
			</div>
		</div>
		...
		<div class="ghl-form-section" style="margin-bottom: 24px;">
			<h3 style="margin: 0 0 16px; font-size: 18px; font-weight: 600; color: #1e293b; display: flex; align-items: center; gap: 8px;">
				<?php esc_html_e( 'Member Association Sync', 'syncly' ); ?>
				<span class="ghl-tooltip-icon" data-ghl-tooltip="<?php esc_attr_e( 'Members, organizers, admins, and moderators are linked to their BuddyBoss groups using GoHighLevel custom object associations. Contact custom fields are no longer required.', 'syncly' ); ?>">?</span>
			</h3>

			<p class="description" style="margin-bottom: 12px;">
				<?php esc_html_e( 'When a user joins or leaves a BuddyBoss group, the plugin queues a background job to add or remove the GoHighLevel association. Group visibility rules still apply, and contacts must already exist in GoHighLevel to be linked.', 'syncly' ); ?>
			</p>

			<ul style="margin: 0 0 1em 1.25em; list-style: disc; color: #475569;">
				<li><?php esc_html_e( 'Association retries are logged so you can resolve missing contacts or permissions.', 'syncly' ); ?></li>
				<li><?php esc_html_e( 'Custom object records include group name, description, type, and URLs for fast lookup inside GoHighLevel.', 'syncly' ); ?></li>
				<li><?php esc_html_e( 'Use the bulk actions below to create objects for every group type and sync existing groups on demand.', 'syncly' ); ?></li>
			</ul>
		</div>
		...
		<div class="ghl-form-section">
			<h3 style="display: flex; align-items: center; gap: 8px;">
				<?php esc_html_e( 'Association Behavior', 'syncly' ); ?>
				<span class="ghl-tooltip-icon" data-ghl-tooltip="<?php esc_attr_e( 'Fine-tune how the plugin handles BuddyBoss groups that are missing metadata and members who have not yet been synced to GoHighLevel.', 'syncly' ); ?>">?</span>
			</h3>
			<p class="description" style="margin-bottom: 16px; color: #475569;">
				<?php esc_html_e( 'Fine-tune how the plugin handles BuddyBoss groups that are missing metadata and members who have not yet been synced to GoHighLevel.', 'syncly' ); ?>
			</p>
			<div style="display: grid; gap: 20px; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr));">
				<div style="border: 1px solid #e2e8f0; border-radius: 12px; padding: 18px; background: #f8fafc;">
					<h4 style="margin: 0 0 10px; color: #1e293b; font-size: 16px; font-weight: 600;">
						<?php esc_html_e( 'Missing Contacts', 'syncly' ); ?>
						<span class="ghl-tooltip-icon" data-ghl-tooltip="<?php esc_attr_e( 'Controls what happens when a BuddyBoss group member doesn\'t have a GoHighLevel contact. Create = automatically sync them to GHL. Skip = log error and wait for manual sync.', 'syncly' ); ?>">?</span>
					</h4>
					<p style="margin: 0 0 14px; color: #475569; font-size: 14px; line-height: 1.5;">
						<?php esc_html_e( 'Choose whether the plugin should automatically create GoHighLevel contacts when a group member is missing one during association.', 'syncly' ); ?>
					</p>
					<div style="display: flex; flex-direction: column; gap: 8px;">
						<label style="display: flex; align-items: center; gap: 8px; font-size: 14px; color: #1e293b;">
							...
							<span><?php esc_html_e( 'Create missing contacts automatically (recommended for fully managed communities)', 'syncly' ); ?></span>
						</label>
						<label style="display: flex; align-items: center; gap: 8px; font-size: 14px; color: #1e293b;">
							<input type="radio" name="buddyboss_missing_contact_strategy" value="skip" <?php checked( 'skip', $missing_contact_strategy ); ?>>
							<span><?php esc_html_e( 'Skip association when contact is missing (manual review required)', 'syncly' ); ?></span>
						</label>
					</div>
				</div>
				<div style="border: 1px solid #e2e8f0; border-radius: 12px; padding: 18px; background: #f8fafc;">
					<h4 style="margin: 0 0 10px; color: #1e293b; font-size: 16px; font-weight: 600;">
						<?php esc_html_e( 'Default Group Type', 'syncly' ); ?>
						<span class="ghl-tooltip-icon" data-ghl-tooltip="<?php esc_attr_e( 'Fallback group type for BuddyBoss groups that have no type assigned. Leave blank to skip untyped groups. Useful if you have legacy groups without types but still want to sync them.', 'syncly' ); ?>">?</span>
					</h4>
					<p style="margin: 0 0 14px; color: #475569; font-size: 14px; line-height: 1.5;">
						<?php esc_html_e( 'Provide a fallback BuddyBoss group type slug for groups that have no type assigned. Leave blank to skip those groups entirely.', 'syncly' ); ?>
					</p>
					<label for="buddyboss_default_group_type" class="screen-reader-text">
						<?php esc_html_e( 'Default BuddyBoss group type', 'syncly' ); ?>
					</label>
					<select 
						...
						style="width: 100%;">
						<option value=""><?php esc_html_e( 'Skip groups without a type', 'syncly' ); ?></option>
		...
		<div class="ghl-form-section">
			<h3 style="display: flex; align-items: center; gap: 8px;">
				<?php esc_html_e( 'Sync Options', 'syncly' ); ?>
				<span class="ghl-tooltip-icon" data-ghl-tooltip="<?php esc_attr_e( 'Control which BuddyBoss groups are synced to GoHighLevel based on visibility and status settings.', 'syncly' ); ?>">?</span>
			</h3>
			...
			<div class="ghl-checkbox-group" style="display: flex; flex-direction: column; gap: 12px;">
				<label class="ghl-checkbox <?php echo ! empty( $settings['buddyboss_sync_private_groups'] ) ? 'is-checked' : ''; ?>" style="display: flex; align-items: center; gap: 8px; margin: 0;">
					...
					<span class="ghl-checkbox-label">
						<?php esc_html_e( 'Sync private groups', 'syncly' ); ?>
						<span class="ghl-tooltip-icon" data-ghl-tooltip="<?php esc_attr_e( 'Includes private BuddyBoss groups in sync operations. When disabled, only public and hidden groups sync to GoHighLevel. Useful for keeping sensitive communities local-only.', 'syncly' ); ?>">?</span>
					</span>
				</label>
				...
				<label class="ghl-checkbox <?php echo ! empty( $settings['buddyboss_sync_hidden_groups'] ) ? 'is-checked' : ''; ?>" style="display: flex; align-items: center; gap: 8px; margin: 0;">
					...
					<span class="ghl-checkbox-label">
						<?php esc_html_e( 'Sync hidden groups', 'syncly' ); ?>
					</span>
				</label>
			</div>
		</div>
		...
		<!-- Bulk Sync Actions -->
		<div class="ghl-form-section">
			<h3><?php esc_html_e( 'Bulk Sync Actions', 'syncly' ); ?></h3>
			<p class="description">
				<?php esc_html_e( 'Use these actions to perform one-time bulk synchronization of existing data.', 'syncly' ); ?>
			</p>
			
			<div class="ghl-bulk-actions">
				<button type="button" class="ghl-button ghl-button-secondary" id="ghl-sync-group-types">
					<span class="dashicons dashicons-admin-customizer"></span>
					<?php esc_html_e( 'Create Custom Objects for Group Types', 'syncly' ); ?>
				</button>
				<button type="button" class="ghl-button ghl-button-secondary" id="ghl-sync-all-groups">
					<span class="dashicons dashicons-groups"></span>
					<?php esc_html_e( 'Sync All Existing Groups', 'syncly' ); ?>
				</button>
...
jQuery(document).ready(function($) {
	...
	$('#buddyboss_groups_enabled').on('change', function() {
		...
		if (isChecked) {
			...
			$labelText.text('<?php echo esc_js( __( 'Enabled', 'syncly' ) ); ?>');
